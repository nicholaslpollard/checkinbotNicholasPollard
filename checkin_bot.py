...
def is_qualifying_checkin(post, instructor_id):
    """
    A post qualifies for a check-in reply only when BOTH are true: it was
    authored by instructor_id, and "check-in" appears anywhere in its title
    (case-insensitive). Tags, body content, exact wording, and dates are
    intentionally never consulted.
    """
    return post.get("author_id") == instructor_id and "check-in" in post.get("title", "").lower()


def has_already_replied(comments, my_user_id):
    """True if any existing comment on the post was authored by my_user_id."""
    return any(comment.get("author_id") == my_user_id for comment in comments)


def process_checkins(client, posts, instructor_id, my_user_id):
    """
    For every qualifying instructor check-in among posts, reply once unless
    my_user_id has already commented. A 423 (check-in closed) is reported
    and skipped rather than treated as a fatal error, so the remaining posts
    still get processed.
    """
    for post in posts:
        if not is_qualifying_checkin(post, instructor_id):
            continue

        comments = client.list_comments(post["id"])
        if has_already_replied(comments, my_user_id):
            print(f"Already replied to check-in {post['id']}; skipping.")
            continue

        response = client.create_comment(post["id"], CHECKIN_REPLY_TEXT)
        if response.status_code == 423:
            print(f"Check-in {post['id']} ('{post['title']}') is not currently "
                  f"accepting replies (423 Locked); skipping.")
            continue

        comment = response.json()
        print(f"Posted check-in reply on post {post['id']} (comment id={comment.get('id')}).")
...
def main():
    ...
    # Check-ins are time-sensitive and cannot be recovered after their daily
    # window closes, so process them before the (retryable) full artifact
    # collection step.
    post_summaries = fetch_instructor_posts(client, config.instructor_id)
    process_checkins(client, post_summaries, config.instructor_id, me["id"])
