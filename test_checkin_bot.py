...
class LoadConfigTests(unittest.TestCase):
    def test_missing_url_raises(self):
        env = {"PRACTICE_API_TOKEN": "abc", "INSTRUCTOR_ID": "7"}
        with patch.dict(os.environ, env, clear=True):
            with self.assertRaises(ConfigError):
                load_config()

    def test_missing_token_raises(self):
        env = {"PRACTICE_API_URL": "https://practice.fhsucyber.com", "INSTRUCTOR_ID": "7"}
        with patch.dict(os.environ, env, clear=True):
            with self.assertRaises(ConfigError):
                load_config()
...
class FakeResponse:
    """Minimal stand-in for a requests.Response used by the check-in tests."""
...
class FakeClient:
    """Minimal stand-in for PracticeHubClient used by the collection/check-in tests."""

    def __init__(self, pages, post_details, attachment_bytes=None,
                 comments_by_post=None, checkin_status_by_post=None):
        ...
        self._comments_by_post = comments_by_post or {}  # {post_id: [comment dict, ...]}
        self._checkin_status_by_post = checkin_status_by_post or {}  # {post_id: status_code}
        self.list_posts_calls = []
        self.create_comment_calls = []  # [(post_id, body), ...]
    ...
    def create_comment(self, post_id, body):
        self.create_comment_calls.append((post_id, body))
        status = self._checkin_status_by_post.get(post_id, 201)
        if status == 423:
            return FakeResponse(423, {"detail": "Locked"})
        return FakeResponse(201, {"id": 900 + post_id, "post_id": post_id, "body": body})
...
class IsQualifyingCheckinTests(unittest.TestCase):
    def test_title_containing_checkin_qualifies_case_insensitively(self):
        post = make_post(1, 7, title="Sept 16 CHECK-IN")
        self.assertTrue(is_qualifying_checkin(post, instructor_id=7))

    def test_checkin_with_surrounding_text_qualifies(self):
        post = make_post(1, 7, title="Check-in for Sept 17")
        self.assertTrue(is_qualifying_checkin(post, instructor_id=7))

    def test_regular_instructor_post_does_not_qualify(self):
        post = make_post(1, 7, title="Weekly announcement")
        self.assertFalse(is_qualifying_checkin(post, instructor_id=7))

    def test_other_authors_checkin_does_not_qualify(self):
        post = make_post(1, 99, title="Student check-in")
        self.assertFalse(is_qualifying_checkin(post, instructor_id=7))


class HasAlreadyRepliedTests(unittest.TestCase):
    def test_my_comment_is_detected_by_author_id(self):
        comments = [{"id": 1, "author_id": 42}, {"id": 2, "author_id": 5}]
        self.assertTrue(has_already_replied(comments, my_user_id=5))

    def test_other_users_comments_do_not_count(self):
        comments = [{"id": 1, "author_id": 42}]
        self.assertFalse(has_already_replied(comments, my_user_id=5))


class ProcessCheckinsTests(unittest.TestCase):
    def test_unanswered_checkin_gets_exactly_one_reply(self):
        post = make_post(1, 7, title="Sept 16 check-in")
        client = FakeClient(pages=[], post_details={}, comments_by_post={1: []})

        process_checkins(client, [post], instructor_id=7, my_user_id=5)

        self.assertEqual(client.create_comment_calls, [(1, CHECKIN_REPLY_TEXT)])

    def test_already_replied_checkin_is_skipped(self):
        post = make_post(1, 7, title="Sept 16 check-in")
        client = FakeClient(
            pages=[], post_details={},
            comments_by_post={1: [{"id": 10, "author_id": 5, "body": "already answered"}]},
        )

        process_checkins(client, [post], instructor_id=7, my_user_id=5)

        self.assertEqual(client.create_comment_calls, [])

    def test_other_users_comment_does_not_block_my_reply(self):
        post = make_post(1, 7, title="Sept 16 check-in")
        client = FakeClient(
            pages=[], post_details={},
            comments_by_post={1: [{"id": 10, "author_id": 999, "body": "a student comment"}]},
        )
    ...
    def test_non_checkin_and_other_author_posts_are_ignored(self):
        posts = [
            make_post(1, 7, title="Regular instructor update"),
            make_post(2, 99, title="Student check-in"),
        ]
        client = FakeClient(pages=[], post_details={}, comments_by_post={1: [], 2: []})

        process_checkins(client, posts, instructor_id=7, my_user_id=5)

        self.assertEqual(client.create_comment_calls, [])

    def test_423_is_reported_and_does_not_stop_remaining_posts(self):
        posts = [
            make_post(1, 7, title="Sept 7 check-in"),   # closed historical check-in
            make_post(2, 7, title="Sept 16 check-in"),  # open graded check-in
        ]
        client = FakeClient(
            pages=[], post_details={},
            comments_by_post={1: [], 2: []},
            checkin_status_by_post={1: 423},
        )
        ...
        self.assertEqual(
            ...
            [(1, CHECKIN_REPLY_TEXT), (2, CHECKIN_REPLY_TEXT)],
        )

    def test_multiple_checkins_processed_in_one_run(self):
        posts = [
            make_post(1, 7, title="Sept 16 check-in"),
            make_post(2, 7, title="Sept 17 check-in"),
        ]
        client = FakeClient(pages=[], post_details={}, comments_by_post={1: [], 2: []})

        process_checkins(client, posts, instructor_id=7, my_user_id=5)

        self.assertEqual(len(client.create_comment_calls), 2)
